...
③ White-glove merge: Ibrahim carried seamlessly into the Printie system. ④ Ferrari-tee: one
sentence · cursor parked · browser CLEANED first. ⑤ Governor: one return per ball, walls stay walls.
**Code's side while hot: dock cards the MOMENT things finish · this file's CDN mirror = the mode's
heartbeat · Ibrahim/Tyler-facing outputs ship to Mac's DM lane immediately on completion.**

## CANARY — order #44464237 / FO 8655675261238
**2026-09-15 12:0x fresh GraphQL read: HOLDS at Printie POD** (`assignedLocation = Printie POD ·
gid://shopify/Location/114083791158` · status OPEN · F#4DRONE-432HZ-BLACK ×1 remaining ·
order UNFULFILLED). **No reversion — no forensics warranted.** Mac's screen read was stale or a
different FO. Bell ① still waits on the completion webhook w/ tracking.

## TWO-TIER PRODUCTION (Deck ruling 2026-09-15 · sources: `data/network_inbound_2026-09-15.json`,
`docs/POOL_ORDER_1_DRAFT.md` v3.1)
- **Tier-1 = the printer network** (ASSIGNMENTS_NOW + PRE_RELEASE, HUMAN-CANON, all lanes check
  back Fri Sept 25): carries base demand — F♯4 450 in · F♯5P 330 · D5 120 · etc. Machine may
  PROPOSE network POs (current asks: F♯5 Single +66; G3-432 DRONE sold-over → Friday-pulse
  replenish; D5 possible over-order trim). Sean Tucker reserve: never auto-triggers.
- **Tier-Printie = the bridge + no-network keys ONLY** (final scope): E4/D4 drones + G3-Oc-440 +
  promise-dated bridge slivers (F♯5 blacks 33/48/47 + pocket whites 3+13 gated). F♯4 family OUT.
  143 firm units ≈ $394. Gates: color verdict → counts/quote → 24+24 shelf check → plates → tap.
- **C♯5 pre-release exists** (Home-Lab beta lanes, 8+5 sold) — when the founder's E5/C♯5 cannon
  talk resumes, those beta lanes are the starting truth.

## API GOLD — RATIFIED AS ARCHITECTURE (Deck 2026-09-15 · spec v2.1.0, contract §0.1)
`batchQuantity` per SKU rule = the pool primitive (Pool #1 rides THEIR rail; workaround retired) ·
`designId` order items = files never travel at order time (Clean Yard gate ① simplifies to the
design-link check) · Pool #1 batch-plate upload = `createPrintieDesign` JSON `modelUrl` lane
(Supabase signed-URL handoff RETIRED for saved-design keys) · replenish thresholds =
Tyler-dashboard-only → the funnel's auto-replenish leg is a PROPOSAL engine (digest → founder tap →
...
SKU migration settles · snorkel watch (B3/D4) each heartbeat.
